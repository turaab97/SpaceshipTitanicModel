...
def target_encode(tr_series, tr_target, val_series, smoothing=10):
    df_enc = pd.DataFrame({'cat': tr_series, 'y': tr_target})
    agg = df_enc.groupby('cat')['y'].agg(['mean','count'])
...
# 4) Load cleaned data
train_raw  = pd.read_csv('train_cleaned.csv', index_col='PassengerId')
test_raw   = pd.read_csv('test_cleaned.csv',  index_col='PassengerId')
...
cat_cols = ['HomePlanet','Destination','CryoSleep','VIP',
            'Title','Deck','Side','SpendBucket','AgeBin','IsAlone','Route','GroupID']
Xa_adv = pd.get_dummies(Xa.fillna('MISSING'), columns=cat_cols)
ya = np.concatenate([np.zeros(len(X_raw)), np.ones(len(X_test_raw))])
adv = RandomForestClassifier(n_estimators=200, random_state=42)
adv.fit(Xa_adv, ya)
imp = pd.Series(adv.feature_importances_, index=Xa_adv.columns).nlargest(10).index.tolist()
to_drop = list({col.split('_')[0] for col in imp})
X_raw.drop(columns=to_drop, inplace=True, errors='ignore')
X_test_raw.drop(columns=to_drop, inplace=True, errors='ignore')
...
# 8) Hyperparameter tuning for base learners
sgkf = StratifiedGroupKFold(n_splits=5, shuffle=True, random_state=42)

hgb_space = {
    'learning_rate':     [0.01,0.03,0.05,0.1],
    'max_leaf_nodes':    [15,31,45,60,100],
...
hgb_search = RandomizedSearchCV(
    ...
    hgb_space, n_iter=30, scoring='roc_auc',
    cv=sgkf.split(X_base, y, groups),
    n_jobs=-1, random_state=42
)
hgb_search.fit(X_base, y)
...
mlp_search = RandomizedSearchCV(
    ...
    mlp_space, n_iter=20, scoring='roc_auc',
    cv=sgkf.split(X_base, y, groups),
    n_jobs=-1, random_state=42
)
mlp_search.fit(X_base, y)
best_mlp = mlp_search.best_estimator_

# 9) Stacking with tuned models
models = {'hgb': best_hgb, 'mlp': best_mlp}
n_train, n_test = len(X_base), len(X_test_base)
...
for tr, va in sgkf.split(X_base, y, groups):
    ...
    for name, mdl in models.items():
        ...
        test_preds[name]    += mdl.predict_proba(X_te)[:,1] / sgkf.n_splits

# 10) Meta-learner & calibration
...
final_prob = sum(test_df[name] * weight for name, weight in zip(models, best_w))
submission = pd.DataFrame({
    'PassengerId': X_test_base.index,
    'Transported': final_prob > best_thr
})
submission.to_csv('submission.csv', index=False)
print("✅ submission.csv ready – thr:", best_thr, "weights:", best_w)
