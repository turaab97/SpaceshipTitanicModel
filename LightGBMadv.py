...
def clean_and_feature(df, is_train=True, kmeans_model=None):
    ...
    # New interaction and binned features
    ...
    df['AgeBin2'] = pd.cut(df['Age'], bins=[0,12,18,30,50,200], labels=['child','teen','young','adult','senior'])
    ...
    numeric_cols = ['Age', 'TotalSpend', 'Age2', 'Age_TotalSpend', 'Spend_per_Age'] + [c for c in df.columns if 'Group_' in c]
...
high_card = ['Title','Deck','Route','AgeBin2']
...
study.optimize(lambda trial: optimize_lgb(trial, X_base, y), n_trials=300)
...
best_thr = 0.5
best_acc = 0
for thr in np.arange(0.45, 0.55, 0.001):
    acc = accuracy_score(y, meta_model.predict_proba(meta_X)[:,1] > thr)
    if acc > best_acc:
        best_acc = acc
        best_thr = thr
