...
def ultra_advanced_filling(df):
    """
    Restore sophisticated clustering with notebook insights
    """
    df = df.copy()
    ...
    # NOTEBOOK INSIGHT: Create total expenses for logical constraints
    df['TotalExpenses'] = df[spend_cols].fillna(0).sum(axis=1)
    
    # NOTEBOOK INSIGHT: Logical CryoSleep filling
    ...
    # RESTORE ORIGINAL CLUSTERING (this was working!)
    ...
    for col in ['CabinDeck', 'CabinSide']:
        if col in df.columns and not df[col].isna().all():
            df[f'{col}_loc_encoded'] = pd.Categorical(df[col]).codes
    ...
    # ENHANCED CLUSTER-BASED FILLING WITH NOTEBOOK INSIGHTS
    
    # Age filling with multiple cluster consensus
    if df['Age'].isna().any():
        missing_age = df['Age'].isna()
        cluster_cols = [col for col in ['Cluster_demo', 'Cluster_behavior', 'Cluster_hierarchical'] if col in df.columns]
    ...
    # ENHANCED spending filling with logical constraints
    for col in spend_cols:
        if df[col].isna().any():
            missing_spend = df[col].isna()
            
            for idx in df[missing_spend].index:
                # NOTEBOOK INSIGHT: Check CryoSleep first
                cryo_status = df.loc[idx, 'CryoSleep']
                if pd.notna(cryo_status) and cryo_status in [True, 'True', 1]:
                    df.loc[idx, col] = 0
                    continue
                
                # Use behavior cluster if available
                if 'Cluster_behavior' in df.columns:
                    ...
                    if not behavior_data[col].empty:
                        # NOTEBOOK INSIGHT: VIP consideration
                        vip_status = df.loc[idx, 'VIP']
                        if pd.notna(vip_status) and vip_status in [True, 'True', 1]:
                            spend_val = behavior_data[col].quantile(0.7)  # VIP spends more
                        else:
            ...
            print(f"✅ Enhanced {col} filling completed")
    ...
    for col in ['HomePlanet', 'Destination']:
        if col in df.columns and df[col].isna().any():
            ...
            cluster_cols = [col for col in ['Cluster_location', 'Cluster_demo'] if col in df.columns]
            
            for idx in df[missing_cat].index:
                filled = False
                
                for cluster_col in cluster_cols:
                    if filled or cluster_col not in df.columns:
                        break
    ...
    for col in ['CabinDeck', 'CabinSide']:
        if col in df.columns and df[col].isna().any():
            ...
            for idx in df[missing_cabin].index:
                ...
                if 'Cluster_location' in df.columns:
...
def main_ensemble_model():
    ...
    # ADVANCED ENSEMBLE STRATEGY for 0.813+
    print("\n🚀 Step 5: ADVANCED Ensemble Training...")
    
    # Strategy 1: Multiple feature selection approaches
    # Mutual Information selection
    mi_selector = SelectKBest(mutual_info_classif, k=min(60, X.shape[1]))
    X_mi = mi_selector.fit_transform(X, y)
    X_test_mi = mi_selector.transform(X_test)
    
    # F-score selection  
    f_selector = SelectKBest(f_classif, k=min(50, X.shape[1]))
    X_f = f_selector.fit_transform(X, y)
    X_test_f = f_selector.transform(X_test)
    
    print(f"   MI features: {X_mi.shape[1]}, F-score features: {X_f.shape[1]}")
    
    # Strategy 2: Diverse base models with different hyperparameters
    base_models_v1 = [
        ('hgb1', HistGradientBoostingClassifier(
            learning_rate=0.02, max_iter=1200, max_leaf_nodes=35,
            min_samples_leaf=20, l2_regularization=0.05, random_state=42)),
        
        ('lgbm1', LGBMClassifier(
            n_estimators=1200, learning_rate=0.025, num_leaves=18,
        ...
        ('cat1', CatBoostClassifier(
            iterations=1000, learning_rate=0.025, depth=5,
            l2_leaf_reg=5, random_seed=42, verbose=False)),
    ]
    
    base_models_v2 = [
        ('hgb2', HistGradientBoostingClassifier(
            learning_rate=0.04, max_iter=800, max_leaf_nodes=25,
            min_samples_leaf=30, l2_regularization=0.15, random_state=123)),
        ...
        ('xgb2', XGBClassifier(
            ...
            reg_alpha=0.1, reg_lambda=0.1, random_state=123, eval_metric='logloss')),
        
        ('et', ExtraTreesClassifier(
            n_estimators=600, max_depth=12, min_samples_split=6,
            min_samples_leaf=3, random_state=123, n_jobs=-1))
    ]
    
    # Strategy 3: Multiple stacking ensembles
    stack1 = StackingClassifier(
        estimators=base_models_v1,
        final_estimator=LogisticRegression(C=0.3, max_iter=3000, random_state=42),
        cv=8, passthrough=True, n_jobs=-1
    )
    
    stack2 = StackingClassifier(
        estimators=base_models_v2,
        final_estimator=RidgeClassifier(alpha=0.5, random_state=42),
        cv=10, passthrough=False, n_jobs=-1
    )
    
    # Strategy 4: Voting ensemble
    voting = VotingClassifier(
        estimators=base_models_v1[:3],  # Use top 3 models
        voting='soft', n_jobs=-1
    )
    
    # Strategy 5: Cross-validation with multiple datasets
    print("\n🧪 Step 6: Advanced Model Evaluation...")
    
    # Test different feature sets
    datasets = [
        ("All Features", X, X_test),
        ("MI Selected", X_mi, X_test_mi), 
        ("F-Score Selected", X_f, X_test_f)
    ]
    ...
    cv = StratifiedKFold(n_splits=7, shuffle=True, random_state=42)  # More folds
    
    for name, X_train_curr, X_test_curr in datasets:
        print(f"\n📊 Testing {name} ({X_train_curr.shape[1]} features)...")
        ...
        for model_name, model in models_to_test:
            try:
                scores = cross_val_score(model, X_train_curr, y, cv=cv, scoring='accuracy', n_jobs=-1)
                mean_score = scores.mean()
                
                print(f"   {model_name}: {mean_score:.5f} ± {scores.std():.5f}")
                
                if mean_score > best_score:
                    best_score = mean_score
                    best_model = model
                    best_X_train = X_train_curr
                    best_X_test = X_test_curr
                    print(f"   🎯 New best: {mean_score:.5f}")
